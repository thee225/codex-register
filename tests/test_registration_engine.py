...
from src.config.constants import OPENAI_API_ENDPOINTS, OPENAI_PAGE_TYPES, EmailServiceType
...
class FakeOpenAIClient:
    def __init__(self, session, sentinel_tokens):
...
def test_run_registers_then_relogs_to_fetch_token(monkeypatch):
    ...
    session_two = QueueSession([
        ...
        (
            "POST",
            OPENAI_API_ENDPOINTS["signup"],
            DummyResponse(payload={"page": {"type": "login_password"}}),
        ),
        (
            "POST",
            PASSWORD_VERIFY_URL,
    ...
    fake_first_client = FakeOpenAIClient(session_one, ["sentinel-1"])
    fake_second_client = FakeOpenAIClient(session_two, ["sentinel-2"])
    email_service = FakeEmailService(["123456", "654321"])
    engine = RegistrationEngine(email_service)
    fake_oauth = FakeOAuthManager()
    engine.http_client = fake_first_client
    engine.oauth_manager = fake_oauth

    monkeypatch.setattr("src.core.register.OpenAIHTTPClient", lambda proxy_url=None: fake_second_client)
    monkeypatch.setattr(
        "src.core.openai.oauth.generate_oauth_url",
        lambda **kwargs: OAuthStart(
            auth_url="https://auth.example.test/flow/2",
            state="state-2",
